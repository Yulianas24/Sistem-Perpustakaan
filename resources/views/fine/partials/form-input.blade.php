<form action="{{ route('denda-buku.store') }}" method="POST">
    ...
    <div class="mb-4">
        ...
        <select id="borrowing_id" name="borrowing_id"
                class="mt-1 block w-full bg-white border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm">
    ...
    <div class="mb-4">
        ...
        <textarea id="description" name="description"
            class="border mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-500 focus:border-gray-500 sm:text-sm">{{ $borrowing->description }}</textarea>
        @error('description')
            <x-input-error-set :message="$message" class="mt-2" />
        @enderror
    </div>
